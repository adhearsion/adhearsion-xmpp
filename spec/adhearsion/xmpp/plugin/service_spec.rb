...
describe Adhearsion::XMPP::Plugin::Service do

  describe "while initializing" do
    after do
      reset_adhearsion_xmpp_config
    end
    ...
    it "should raise an exception when no server has been configured" do
      Adhearsion.config[:adhearsion_xmpp].jid = "usera"
      Adhearsion.config[:adhearsion_xmpp].password = "p"
      Adhearsion.config[:adhearsion_xmpp].server = ""
    ...
    it "should raise an exception when an invalid port has been configured" do
      Adhearsion.config[:adhearsion_xmpp].jid = "usera"
      Adhearsion.config[:adhearsion_xmpp].password = "p"
      Adhearsion.config[:adhearsion_xmpp].port = "52222"
  ...
  describe "when starting the XMPP connection" do
    ...
    it "should call Connection.start method with the valid parameters" do
      Adhearsion::XMPP::Plugin::Connection.should_receive(:start).with("usera", "p", "localhost", 5222).and_return true
